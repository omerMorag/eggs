...
import PreDecisionAccordions from "@/components/where-to-go-2/PreDecisionAccordions";
...
/**
 * "איפה כדאי לעשות?" — WHERE TO DO 2.0. הירו קצר + כלי בחירה הדרגתי
 * (WhereToDoTool: אזור → סוג מסלול → קופה → תוצאות → השוואה → בחירה),
 * ואז תמצית לרופאים פרטיים. כל התוכן ההסברתי הישן (בורר 4 מסלולי המימון
 * + כל הטבלאות) עדיין קיים במלואו — שום שורה לא נמחקה — רק זז לתחתית תחת
 * "לפני שאת בוחרת" (PreDecisionAccordions), מקופל כברירת מחדל.
 */
export default function WhereToGoSection({ progress }: WhereToGoSectionProps) {
  ...
  return (
    <div className="print-stack animate-fadeUp">
      ...
      {/* כלי הבחירה ההדרגתי — §2-§11 */}
      <section className="mt-6 sm:mt-8">
        <WhereToDoTool progress={progress} />
      </section>

      {/* כל התוכן ההסברתי הקודם — §14, מקופל ומועבר לתחתית */}
      <PreDecisionAccordions>
        {/* בורר ארבעת המסלולים */}
        <section>
